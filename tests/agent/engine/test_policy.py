...
def test_base_shapes_are_permissive_no_ops() -> None:
    ...
    assert gate.check(
        session, asst, ["note", "peek"], turn=0, compaction_imminent=False,
    ) is None
    gate.on_turn_complete()

    assert policy_mod.DispatchGuard().check(session, _tc("tap"), turn=0) is None
    assert policy_mod.ResultObserver().observe(
        session, _tc("tap"), changed=True, failed=False,
    ) is None
...
def test_compaction_checkpoint_passes_a_note_with_scratchpad() -> None:
    ...
    asst = _asst([
        _tc("note", {"summary": "x", "scratchpad": "cart=3"}), _tc("peek"),
    ])

    rej = gate.check(
        Session(), asst, ["note", "peek"], turn=5, compaction_imminent=True,
    )
...
def test_compaction_checkpoint_rejects_once_then_renews_on_turn_complete() -> None:
    ...
    first = gate.check(Session(), bare, ["note", "peek"], turn=5, compaction_imminent=True)
    second = gate.check(Session(), bare, ["note", "peek"], turn=5, compaction_imminent=True)
    ...
    gate.on_turn_complete()
    third = gate.check(Session(), bare, ["note", "peek"], turn=6, compaction_imminent=True)
...
def test_memory_cue_scan_reads_update_progress_fields() -> None:
    ...
    asst = _asst([
        ...
        _tc("update_progress", {
            ...
            "steps": [{"content": "reply", "status": "in_progress"}, "not-a-dict"],
        }),
    ])
...
def test_memory_cue_scan_skipped_when_disabled(monkeypatch) -> None:
    ...
    gate = next(
        g for g in policy_mod.default_policies(layout_incomplete=False).turn_gates
...
def test_layout_lint_passes_when_lint_finds_nothing(mocker) -> None:
    ...
    block = policy_mod.LayoutLint().check(
        Session(), _tc("long_press", {"bbox": [0, 0, 1, 1]}), turn=0,
    )
...
def test_default_policies_declares_the_documented_order() -> None:
    ...
    assert [type(g).__name__ for g in p.turn_gates] == [
        "CompactionCheckpoint", "PitfallCheckpoint", "MemoryCueCheckpoint",
    ]
    assert [type(g).__name__ for g in p.dispatch_guards] == [
        "PlanGate", "LayoutLint", "StuckBlock",
    ]
    assert [type(o).__name__ for o in p.result_observers] == [
        "KeyboardBelief", "StuckRecorder",
    ]
