...
- `save_memory`: only "remember this", a lasting preference, or a learned app / entry→app mapping (AGENT § Choose apps) — session detail goes to the daily log. The engine nudges once at close if you flagged a `remember this` cue but never saved.
