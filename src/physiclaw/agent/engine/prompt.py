...
def _render_examples() -> list[str]:
    ...
    return [
        ...
        "",
        "**`home_screen` on home opens the App Switcher** — its swipe-up goes home from an app, but on the home screen opens the App Switcher.",
        "❌ In the App Switcher → `home_screen` again → swipes up → App Switcher again, STUCK loop.",
        "✅ In the App Switcher → `tap(bbox=[0.4, 0.9, 0.6, 0.95])` (empty area) to reach home. Never re-call `home_screen`.",
        "",
