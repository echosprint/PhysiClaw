...
async def freeze_snapshot(request, physiclaw: "PhysiClaw", state: AnnotationState):
...
async def get_frozen_snapshot(request, physiclaw: "PhysiClaw", state: AnnotationState):
...
async def handle_confirm(request, state: AnnotationState, physiclaw: "PhysiClaw"):
