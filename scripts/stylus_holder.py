...
def dim_h(x1, x2, y, text, side="below", offset=12 * mm):
    ...
    c.line(x1, dy, x2, dy)
    draw_arrow(x1, dy, 0)
    draw_arrow(x2, dy, 180)
    c.setFont("Helvetica", DIM_FONT)
    tw = c.stringWidth(text, "Helvetica", DIM_FONT)
    mx = (x1 + x2) / 2
    c.setFillColor(white)
    c.rect(mx - tw / 2 - 2, dy - 4, tw + 4, 8, fill=1, stroke=0)
    c.setFillColor(BLUE)
    c.drawCentredString(mx, dy - 2.5, text)
...
def dim_v(y1, y2, x, text, side="right", offset=12 * mm):
    ...
    c.rotate(90)
    c.setFillColor(white)
    c.rect(-tw / 2 - 2, -4, tw + 4, 8, fill=1, stroke=0)
    c.setFillColor(BLUE)
...
c.drawCentredString(
    W / 2, H - MARGIN - 52, "Front View  |  Dimensions in mm  |  Stainless Steel"
)
if HAS_CJK:
    c.setFont("CJK", 9)
...
c.drawString(
    MARGIN + 4,
    H - MARGIN - 84,
    "M3 external thread φ 3 × 5  —  Rod φ 2.5 × 75  —  Knurled Tube φ 9 / φ 7 × 100",
)
if HAS_CJK:
    ...
    x = MARGIN + 4
    c.setFont("CJK", 9)
    c.drawString(x, y_cn, "M3 外螺纹")
    x += c.stringWidth("M3 外螺纹", "CJK", 9)
    c.setFont("Helvetica", 9)
    c.drawString(x, y_cn, " φ 3 × 5  —  ")
    x += c.stringWidth(" φ 3 × 5  —  ", "Helvetica", 9)
    c.setFont("CJK", 9)
    c.drawString(x, y_cn, "钢棒")
    x += c.stringWidth("钢棒", "CJK", 9)
    c.setFont("Helvetica", 9)
    c.drawString(x, y_cn, " φ 2.5 × 75  —  ")
    x += c.stringWidth(" φ 2.5 × 75  —  ", "Helvetica", 9)
    c.setFont("CJK", 9)
    c.drawString(x, y_cn, "滚花管材")
    x += c.stringWidth("滚花管材", "CJK", 9)
    c.setFont("Helvetica", 9)
...
c.setFont("Helvetica", DIM_FONT)
c.drawString(
    ldr_end - c.stringWidth("φ 7 (ID)", "Helvetica", DIM_FONT) - 2, id_y - 3, "φ 7 (ID)"
)
c.setStrokeColor(black)
c.setFillColor(black)
...
c.translate(mg_x, mid_mg)
c.rotate(90)
tw = c.stringWidth("2", "Helvetica", DIM_FONT)
c.setFillColor(white)
c.rect(-tw / 2 - 2, -4, tw + 4, 8, fill=1, stroke=0)
c.setFillColor(BLUE)
c.drawCentredString(0, -2.5, "2")
c.restoreState()
c.setStrokeColor(black)
c.setFillColor(black)
